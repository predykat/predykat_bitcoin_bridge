...
use crate::{bitcoin_api, ecdsa_api, types};
...
async fn build_transaction(
    ...
    loop {
        let transaction =
            build_transaction_with_fee(own_utxos, own_address, dst_address, amount, total_fee, &dist_address)
...
fn build_transaction_with_fee(
    ...
    if dist_address.is_some() {
        outputs.push(TxOut {
            script_pubkey: Script::new_op_return(b"dupa"),
            value: 0
        });
    }
...
pub async fn build_bridge_transaction(
    network: Network,
    key_name: String,
    derivation_path: Vec<Vec<u8>>,
    signer_public_key: String,
    signer_address_string: String,
    amount: u64,
    dist_network: types::BridgeNetwork,
    dist_address: String
) -> String {
    let signer_public_key: &[u8] = &hex::decode(signer_public_key).unwrap();
    // Get fee percentiles from previous transactions to estimate our own fee.
    let fee_percentiles = bitcoin_api::get_current_fee_percentiles(network).await;
    let fee_per_byte = if fee_percentiles.is_empty() {
        // There are no fee percentiles. This case can only happen on a regtest
    ...
    print("Fetching UTXOs...");
    let signer_utxos = bitcoin_api::get_utxos(network, signer_address_string.clone())
        .await
        .utxos;
    //APT 1e1ae2e1e5b7bddf7b0d863e4daa70ae988acdc3471f2e6a5faa41a68884b418

    let own_address = get_p2pkh_address(network, key_name, derivation_path).await;
    let bridge_address = Address::from_str(&own_address).unwrap();
    let signer_address = Address::from_str(&signer_address_string).unwrap();

    let network_symbol = match dist_network {
        types::BridgeNetwork::Aptos => String::from("APT ")
    };
    let dist_path = network_symbol + &dist_address;
