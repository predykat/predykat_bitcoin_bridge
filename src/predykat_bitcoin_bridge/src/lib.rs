...
pub async fn get_utxos(address: String) -> GetUtxosResponse {
    let network = NETWORK.with(|n| n.get());
    let utxos: GetUtxosResponse = bitcoin_api::get_utxos(network, address).await;
    print(&format!("TX id: {}", hex::encode(utxos.utxos[0].outpoint.txid.to_vec())));
    return utxos
}
...
#[update]
pub async fn build_bridge_transaction(request: types::BuildBridgeTransactionRequest){
    let derivation_path = DERIVATION_PATH.with(|d| d.clone());
    let network = NETWORK.with(|n| n.get());
    let key_name = KEY_NAME.with(|kn| kn.borrow().to_string());
    bitcoin_wallet::build_bridge_transaction(
        network,
        key_name,
        derivation_path,
        ...
        request.amount_in_satoshi,
        request.dist_network,
        request.dist_address,
    ).await;
}
